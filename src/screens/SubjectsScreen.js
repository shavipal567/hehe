        ...
        <ScrollView>
          ...
          <View style={styles.paletteRow}>
            {BACKGROUND_PALETTES.map((p) => (
              <TouchableOpacity
                key={p.id}
                style={[styles.paletteSwatch, bgPalette === p.id && styles.paletteSwatchActive]}
                onPress={() => setBgPalette(p.id)}
              >
                <View style={styles.paletteColors}>
                  <View style={[styles.paletteHalf, { backgroundColor: p.swatch[0] }]} />
                  <View style={[styles.paletteHalf, { backgroundColor: p.swatch[1] }]} />
                </View>
                <Text style={styles.paletteLabel}>{p.label}</Text>
                {bgPalette === p.id && <Text style={styles.paletteCheck}>✓</Text>}
              </TouchableOpacity>
            ))}
          </View>
        </ScrollView>
...
function makeStyles(theme, darkMode) {
  ...
  paletteSwatch: {
    width: "31%", backgroundColor: theme.cardBg, borderRadius: 14, padding: 10, alignItems: "center",
    borderWidth: 2, borderColor: "transparent",
  },
  paletteSwatchActive: { borderColor: theme.primary },
  paletteColors: { flexDirection: "row", width: "100%", height: 36, borderRadius: 8, overflow: "hidden", marginBottom: 6 },
  paletteHalf: { flex: 1 },
